...
const SERVICES = [
  {
    ...
    centreRows: [
      { label: 'Visit completion', val: '98.2%', pct: 98,  grad: 'linear-gradient(90deg,#223A5E,#0C8B7A)' },
      { label: 'Same-day slots',   val: '100%',  pct: 100, grad: 'linear-gradient(90deg,#0C8B7A,#0a6b5e)' },
    ],
    icon: (
  ...
  },
  {
    ...
    centreRows: [
      { label: 'Calls answered',  val: '< 60s', pct: 97, grad: 'linear-gradient(90deg,#1A56DB,#0f3a8a)' },
      { label: 'Resolution rate', val: '94%',   pct: 94, grad: 'linear-gradient(90deg,#0C8B7A,#223A5E)' },
    ],
    icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
  ...
  },
  {
    ...
    centreRows: [
      { label: 'Sessions booked', val: '10K+/mo', pct: 90, grad: 'linear-gradient(90deg,#7C3AED,#4c1d95)' },
      { label: 'Return rate',     val: '87%',     pct: 87, grad: 'linear-gradient(90deg,#0C8B7A,#223A5E)' },
    ],
    icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
  ...
  },
  {
    ...
    centreRows: [
      { label: 'Rx issued',          val: '50K+',  pct: 99, grad: 'linear-gradient(90deg,#0C8B7A,#C97B1A)' },
      { label: 'Pharmacy delivery',  val: '< 2hrs', pct: 95, grad: 'linear-gradient(90deg,#0C8B7A,#065f52)' },
    ],
    icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
  ...
  },
  {
    ...
    centreRows: [
      { label: 'Data dashboards', val: 'Live',  pct: 100, grad: 'linear-gradient(90deg,#C97B1A,#7a4a10)'  },
      { label: 'Report accuracy', val: '99.9%', pct: 100, grad: 'linear-gradient(90deg,#223A5E,#0C8B7A)' },
    ],
    icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
  ...
  },
  {
    ...
    centreRows: [
      { label: 'Certs issued',    val: '< 1hr', pct: 95,  grad: 'linear-gradient(90deg,#223A5E,#0C8B7A)' },
      { label: 'Compliance rate', val: '100%',  pct: 100, grad: 'linear-gradient(90deg,#0C8B7A,#065f52)' },
    ],
    icon: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
...
const DEFAULT_CENTRE = {
  ...
  rows: [
    { label: 'Visit completion',   val: '98.2%', pct: 98,   grad: 'linear-gradient(90deg,#223A5E,#0C8B7A)' },
    { label: 'Prescription accuracy', val: '99.7%', pct: 99.7, grad: 'linear-gradient(90deg,#0C8B7A,#C97B1A)' },
  ],
};
...
export default function CorporateHub() {
  ...
  const placeCards = useCallback((skipTransition = false) => {
    ...
    cards.forEach((el, i) => {
      ...
      let scale   = 0.85 + 0.15 * (1 - depthFactor);
      let opacity = 0.6  + 0.4  * (1 - depthFactor);

      if (activeIdxRef.current === i) { scale = 1.06; opacity = 1; }
      ...
      el.style.zIndex    = Math.round(opacity * 10) + (activeIdxRef.current === i ? 20 : 0);
  ...
  const previewCard = useCallback((idx) => {
  ...
  const rotateTo = useCallback((target, onDone) => {
  ...
  /* ── Activate card (click) ── */
  const activateCard = useCallback((idx) => {
    ...
    cardEls.current.forEach((el, i) => el?.classList.toggle('hc-sat-active', i === idx));
  ...
  <ECGTimeline />
      ...
      >
        ...
        <div className="hc-hub-centre">
          <div className={`hc-hub-centre-inner${centreFade ? ' hc-fading' : ''}`}>
        ...
        <div className="hc-hub-orbit" ref={orbitRef}>
          {SERVICES.map((svc, i) => (
            ...
            >
              <div className="hc-sat-accent" style={{ background: svc.accent }} />
