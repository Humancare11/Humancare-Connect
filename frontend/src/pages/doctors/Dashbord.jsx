...
const MOCK_APPOINTMENTS = [
  {
    ...
    type: "Video",
    status: "upcoming",
    issue: "Chest pain checkup",
  },
  {
    id: 2,
    patient: "Vikram Patil",
    time: "11:30 AM",
    date: "Today",
    type: "In-Clinic",
    status: "upcoming",
  ...
  {
    id: 6,
    patient: "Pradeep More",
    time: "3:00 PM",
    date: "Yesterday",
    type: "Video",
    status: "done",
    issue: "Diabetes review",
  },
];
...
const typeIcon = (type) => {
  if (type === "Video") {
    return (
      <span className="dd-type-icon video">
        <Ico.Video />
      </span>
    );
  }
  ...
  if (type === "Chat") {
    return (
      <span className="dd-type-icon chat">
        <Ico.Chat />
      </span>
    );
  }
  ...
  return (
    <span className="dd-type-icon clinic">
      <Ico.Clinic />
    </span>
  );
};
...
export default function Dashbord() {
  ...
  const initials = profileDoctor.name
    ? profileDoctor.name
        .split(" ")
        .map((w) => w[0])
        .join("")
        ...
        {STATS.map((s) => (
          <div className="dd-stat-card" key={s.label}>
            ...
            <div className="dd-stat-info">
              <div className="dd-stat-val" style={{ color: s.color }}>
                {s.val}
              </div>
      ...
      <div className="dd-grid">
              ...
              todayAppointments.map((a) => (
                <div className="dd-appt-item" key={a.id}>
                  ...
                  <div className="dd-appt-actions">
                    {a.status === "upcoming" ? (
                      <>
                        <button className="dd-appt-btn dd-appt-btn--start">
                          Start
                        </button>
                        <button className="dd-appt-btn dd-appt-btn--cancel">
                          Cancel
                        </button>
                      </>
                    ) : (
        ...
        <div className="dd-right-col">
          <div className="dd-card dd-profile-card">
            ...
            <div className="dd-profile-body">
              ...
              <div className="dd-profile-name">
                {profileDoctor.name || "Doctor"}
              </div>
              <div className="dd-profile-spec">
                {profileDoctor.specialty || "General Physician"}
              </div>

              <div className="dd-profile-meta">
                <span>📧 {profileDoctor.email || "doctor@example.com"}</span>
                {profileDoctor.experience && (
                  <span>💼 {profileDoctor.experience} yrs exp</span>
                )}
                {profileDoctor.hospital && (
                  <span>🏥 {profileDoctor.hospital}</span>
                )}
                {profileDoctor.licenseNumber && (
                  <span>📋 {profileDoctor.licenseNumber}</span>
                )}
              </div>
          ...
          <div className="dd-card dd-quick-card">
            <h2 className="dd-card-title" style={{ marginBottom: 14 }}>
              Quick Actions
            </h2>
